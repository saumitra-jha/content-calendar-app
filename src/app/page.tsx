...
export default function Home() {
  ...
  // Map: date string (YYYY-MM-DD) -> scheduled content array
  ...
  useEffect(() => {
    async function fetchScheduled() {
      ...
      const { data, error } = await supabase
        ...
        .lte("date", toDate);
      setFetching(false);
      console.log('inside fetchScheduled');
      if (error) {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      if (isSignedIn) {
        // Get the JWT for the 'supabase' template
        const token = await getToken({ template: 'supabase' });
  ...
  // Minimal insert test for debugging
  // useEffect(() => {
  //   const testInsert = async () => {
  //     if (user) {
  //       const supabase = getSupabaseClient(await getToken({ template: 'supabase' }) || undefined);
  //       const { data, error } = await supabase
  //         .from("scheduled_content")
  //         .insert([
  //           { user_id: user.id, content: "test", date: "2025-07-01" }
  //         ]);
  //       console.log("Minimal insert test result:", { data, error });
  //     }
  //   };
  //   testInsert();
  ...
  // DroppableDay component (moved inside to access handleDelete)
        ...
        {scheduled.length > 0 ? (
          <div className="flex flex-col gap-1 w-full mt-1">
            {scheduled.map((item, idx) => (
              <span
                key={item.id}
                className={`flex items-center justify-between text-xs font-semibold text-left break-words rounded px-2 py-1 border ${SCHEDULE_COLORS[idx % SCHEDULE_COLORS.length]} shadow-sm w-full`}
              >
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-4">
      {/* User profile button in top right for signed-in users only */}
      ...
      <SignedIn>
        ...
        {/* Main app content (prompt, calendar, etc.) */}
        {!submittedIdea ? (
